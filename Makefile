...
SRC = $(SRC_DIR)checks.c $(SRC_DIR)main.c $(SRC_DIR)keycode.c $(SRC_DIR)math.c \
	...
	$(SRC_DIR)event_handler.c $(SRC_DIR)minimap.c $(SRC_DIR)perp_distance.c \
	$(SRC_DIR)draw_wall.c
OBJ = $(SRC:$(SRC_DIR)%.c=%.o)

LIB_INC = -Iincludes -Ilibft/includes -I ~/SDL2.framework/Headers `pkg-config --cflags SDL2` `pkg-config --cflags SDL2_mixer`
#-I$(LIB)
FLAGS += -Llibft -lft -lm `pkg-config --libs SDL2` `pkg-config --libs SDL2_mixer`
