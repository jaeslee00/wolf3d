...
/*   Updated: 2019/08/29 14:12:31 by viccarau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	minimap(t_wolf *wolf)
{
	...
	while (y < map_height)
	{
		...
		while (x < map_width)
		{
			...
			wolf->player.m[x + y * minimap_width].x = x * zoom + W / 2 - zoom * minimap_width / 2;
			wolf->player.m[x + y * minimap_width].y = y * zoom + H / 2 - zoom * minimap_height / 2;

			wolf->player.m[(x + 1) + y * minimap_width].x = (x + 1) * zoom + W / 2 - zoom * minimap_width / 2;
			wolf->player.m[(x + 1) + y * minimap_width].y = y * zoom + H / 2 - zoom * minimap_height / 2;

			wolf->player.m[x + (y + 1) * minimap_width].x = x * zoom + W / 2 - zoom * minimap_width / 2;
			wolf->player.m[x + (y + 1) * minimap_width].y = (y + 1) * zoom + H / 2 - zoom * minimap_height / 2;

			wolf->player.m[(x + 1) + (y + 1) * minimap_width].x = (x + 1) * zoom + W / 2 - zoom * minimap_width / 2;
			wolf->player.m[(x + 1) + (y + 1) * minimap_width].y = (y + 1) * zoom + H / 2 - zoom * minimap_height / 2;

			if (wolf->obj.nb[x + y * map_width] == 1)
			{
				wolf->player.m[x + y * minimap_width].type = 1;
				wolf->player.m[(x + 1) + y * minimap_width].type = 1;
				wolf->player.m[x + (y + 1) * minimap_width].type = 1;
				wolf->player.m[(x + 1) + (y + 1) * minimap_width].type = 1;
			}
			else if (wolf->obj.nb[x + y * map_width] == 0)
			{
				if (wolf->player.m[x + y * minimap_width].type != 1)
					wolf->player.m[x + y * minimap_width].type = 0;
				if (wolf->player.m[x + 1 + y * minimap_width].type != 1)
					wolf->player.m[x + 1 + y * minimap_width].type = 0;
				if (wolf->player.m[x + (y + 1) * minimap_width].type != 1)
					wolf->player.m[x + (y + 1) * minimap_width].type = 0;
				if (wolf->player.m[x + 1 + (y + 1) * minimap_width].type != 1)
					wolf->player.m[x + 1 + (y + 1) * minimap_width].type = 0;
			}
			x++;
		}
		y++;
